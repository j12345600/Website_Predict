...
function querry(indice){
   ...
   data={
      ...
      total:parseInt(dataInJson["retVal"][newSi]['tot']),
      district:dataInJson["retVal"][newSi]['sarea'],
      lat:dataInJson["retVal"][newSi]['lat'],
      lng:dataInJson["retVal"][newSi]['lng'],
      addr:dataInJson["retVal"][newSi]['ar'],
      districtE:dataInJson["retVal"][newSi]['sareaen'],
      nameE:dataInJson["retVal"][newSi]['snaen'],
      addrE:dataInJson["retVal"][newSi]['aren']
   };
   connection.query('INSERT IGNORE INTO Station_Info SET ?',data,function(err,result){
      if(err) {
         console.log(err);
      }
   });
	...
	// 	toDate(dataInJson["retVal"][newSi]['mday']),indice],function(err,result){
	// 	if(err) {
	// 	   console.log(err);
	// 	}
   // 	});
...
request(url1,{encoding:null},function(error,response,body){
